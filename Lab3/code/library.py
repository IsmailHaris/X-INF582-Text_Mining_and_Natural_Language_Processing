...
def compute_node_centrality(graph):
    ...
    # weighted degree: .strength() method with 'weights' argument
    w_degrees = graph.strength(weights="weight")
    w_degrees = [round(float(degree)/(len(graph.vs)-1),5) for degree in w_degrees]

    # closeness: .closeness() method with 'normalized' argument set to True
    closeness = graph.closeness(normalized=True)
    closeness = [round(value,5) for value in closeness]

    # weighted closeness: same as above, but with 'weights' argument
    w_closeness = graph.closeness(normalized=True, weights="weight")
