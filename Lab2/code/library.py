...
def clean_text_simple(text, my_stopwords, punct, remove_stopwords=True, pos_filtering=True, stemming=True):
    ...
    if pos_filtering == True:
        ...
        for item in tagged_tokens:
            if item[1] in ['NN','NNS','NNP','NNPS','JJ','JJS','JJR']:
                tokens_keep.append(item[0])
        tokens = tokens_keep
    if remove_stopwords:
        tokens = [t for t in tokens if t not in my_stopwords] # filter out stopwords.
    if stemming:
...
def unweighted_k_core(g):
    ...
    while len(gg.vs)>0:
        while [deg for deg in gg.strength() if deg <= i]:
            ...
            # incident edges on the deleted vertex are automatically removed
        i += 1
